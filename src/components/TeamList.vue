...
<template>
  <div class="d-flex flex-md-row flex-column justify-content-around align-items-center w-100">
    <div class="addPlayerModal">
      <input class="addPlayerInputText" v-model="player" placeholder="Add a new player" />
      <br /><br />
    ...
    <div v-if="this.players.length" class="list justify-content-center align-items-center">
      <ul class="list">
        <li
          ...
          :key="i"
          class="d-flex align-items-center justify-content-between w-100 fs-4 pe-3 m-1"
        >
          {{ i + 1 }} - {{ player.length > 8 ? player.substring(0, 8) + '...' : player }}
          <button class="btn btn-outline-danger" @click="deletePlayer(i)">Remove</button>
        </li>
      </ul>
    </div>
